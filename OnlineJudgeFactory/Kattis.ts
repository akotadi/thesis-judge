    ...
    try {
      ...
      await page.waitForFunction(
        ({ table, KattisVeredicts }) => {
          try {
            const veredictHTMLString =
              table.childNodes[0].childNodes[3].textContent.toLowerCase();
            if (
              ...
              veredictHTMLString.includes(
                KattisVeredicts.TIME_LIMIT_EXCEEDED
              ) ||
      ...
      const status = await page.$(
        'table[id="judge_table"] > tbody > tr:nth-child(1) > td:nth-child(4)'
      );
      // For the table, get the element that is in the first row, column five (time of the submission)
      const time = await page.$(
        'table[id="judge_table"] > tbody > tr:nth-child(1) > td:nth-child(5)'
      );
