  ...
  async getSubmissionVeredict(page: Page): Promise<ProblemVeredict> {
    try {
      ...
      const status = await page.$(
        'table[class="status-frame-datatable"] > tbody > tr:nth-child(2) > td:nth-child(6)'
      );
      ...
      const time = await page.$(
        'table[class="status-frame-datatable"] > tbody > tr:nth-child(2) > td:nth-child(7)'
      );
      // For the table, get the element that is in the first row, column eight (memory of the submission)
      const memory = await page.$(
        'table[class="status-frame-datatable"] > tbody > tr:nth-child(2) > td:nth-child(8)'
      );
